...
describe ConfigParser do
  ...
  it "should return raise error if compound setting has undefined link" do
    ...
    ENV[ OPTS]="test_non_exist_compound=${#{non_exist_setting_name}}/abdc"
  ...
  it "should raise exception when it gets unamed parameter" do
    invalid_pair = "${temp}/folder"
    ENV[ OPTS]="temp=C:/abcd,#{invalid_pair}"
    message  = "Invalid pair: #{invalid_pair}. Expected: name=value"
    lambda{ConfigParser.new()}.should raise_error(ArgumentError, message )
  end
  
  it "should return empty string if it gets empty value" do
    ENV[ OPTS]="temp=,abcd=qwerty"
    @settings.instance_variable_set(:@env_properties, ConfigParser.new())
